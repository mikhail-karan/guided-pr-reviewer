...
<button
	...
	class="flex cursor-pointer items-center gap-2 rounded-lg bg-blue-600 px-8 py-2 font-bold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
>
	{#if isSubmitting}
...
{#if submitError}
	<div class="mt-4 rounded-lg border border-red-200 bg-red-50 p-4">
		...
		<button
			...
			class="mt-2 cursor-pointer text-xs text-red-600 underline hover:text-red-800"
		>
			Dismiss
		</button>
	</div>
...
{#if submitSuccess}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
		<div class="rounded-xl bg-white p-8 text-center shadow-xl">
			<svg
			...
			<p class="mt-2 text-gray-500">Redirecting to PR list...</p>
		</div>
	</div>
{/if}
```
...
<fieldset disabled={isSubmitting}>
	...
	<!-- Feedback textarea -->
	<textarea bind:value={reviewBody} disabled={isSubmitting} class="..."></textarea>
</fieldset>
```
...
<div class="border-t border-gray-100 pt-4">
	{#if submitError}
		<div
			class="mb-4 flex items-start justify-between rounded-lg border border-red-200 bg-red-50 p-4"
		>
			<div class="flex items-center gap-2 text-red-700">
	...
	<div class="flex justify-end gap-4">
		<button
			class="cursor-pointer rounded-lg border border-gray-300 px-6 py-2 font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50"
			disabled={isSubmitting}
		>
			Export as Markdown
		</button>
		<button
			onclick={submitReview}
			disabled={isSubmitting}
			class="flex min-w-[200px] cursor-pointer items-center justify-center gap-2 rounded-lg bg-blue-600 px-8 py-2 font-bold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
		>
			{#if isSubmitting}
				<svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
					<circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"
